...
import { type WallHistoryListEntry, type TimeFilter } from './data';
...
export function QuestionHistory({
  ...
  const filteredHistory = useMemo(() => {
    ...
    if (timeFilter === '30d') return entries.filter((h) => h.bucket === '30d');
    return entries.filter((h) => h.bucket === '30d' || h.bucket === '90d');
  }, [timeFilter, entries]);
    ...
    <section className="bg-black px-6 sm:px-12 lg:px-[80px] py-16 lg:py-4">
      <div className="max-w-[1440px] mx-auto">
        <div className={HISTORY_CONTENT_RAIL}>
          ...
          {entries.length === 0 ? (
            <p className={`mt-10 text-secondary-200 ${isRtl ? 'font-arabic text-right' : 'font-helvetica'}`}>
              {t('historyEmpty')}
            </p>
          ) : (
            <>
            ...
            {visibleHistory.map((item) => {
              ...
              const weekN = Number.isNaN(d.getTime()) ? 0 : getISOWeek(d);
              return (
